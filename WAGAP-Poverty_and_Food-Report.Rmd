...
#  select(food_problem_lack_of_transportation:food_problem_not_enough_income) %>% 
  ...
  ggplot(aes(area = n,
             ...
             label = plot_label)) +
  geom_treemap() +
  geom_treemap_text(aes(fontface = "italic", colour = "white", start = "center", grow = T, family = "Georgia")) +
  
# QUESTION: how can I set the colors used in my treemap?  scale_fill gives Error: Continuous value supplied to discrete scale.  Also, why doesn't colour = "white" change the label color?   
  # scale_fill_brewer(palette = "Set3") +
  theme(plot.title = element_markdown()) +
#  theme(plot.title = element_text(color = "blue", family = "Helvetica")) +
  theme(legend.text = element_text(family = "Georgia")) +
  theme(legend.position = "bottom") + 
  labs(title = "Food problem are dominated by lack of <span style = 'color: orange'>income</span>", caption = "treemap caption draft")

# QUESTION: I'd like to use my custom theme or another theme like theme_ipsum_rc.... does treemapify not work with custom themes??
theme_ipsum_rc()
