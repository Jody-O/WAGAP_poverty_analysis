...
summary_food_plot <- food_challenges_by_ethnicity %>% 
  ggplot(aes(y = is_food_a_challenge,
             ...
             fill = race_ethnicity)) +
  geom_col() + 
# NOTE to self: how can I format the percent that shows up in the label (to make it say "21.9%" instead of the decimal)? ANSWER: use scales::percent
  geom_label(aes(label = scales::percent(pct_of_all_challenge_level_responses, accuracy = 0.1),
...
# QUESTION: Why don't the axis titles show up as Georgia font when the axis text and plot title do?  NOTE - I am using Windows. To import 'Georgia' I used: library(extrafont)
  #font_import(pattern = "Georgia")
  # windowsFonts(`Georgia` = windowsFont("Georgia"))
        panel.grid.minor.y = element_blank(),
        panel.grid.major.x = element_line(color = "light blue"))
