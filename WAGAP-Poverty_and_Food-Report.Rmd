...
# QUESTION: is there a way to 'grow' the label size in relation to the area value (so "not enough income to purchase food" is the largest font size and "other reasons" is the smallest font size?)
  
# NOTE to self: for some reason label color & other specifications must be set outside of the aes
  scale_fill_distiller(palette = "YlGnBu",
                       trans = "reverse", name = "number of \nresponses") +
  # trans changes the order of the color scale
  guides(fill = guide_colorbar(reverse = TRUE)) +
  
#QUESTION:  How can I change the fill scale max/min values? - I'd like the plot colors to be a bit less extreme from the high to low end of the scale.  Also it would be good to know how to change the scale to go from 25-350 or so - instead of using the default max/min
